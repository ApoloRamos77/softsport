    ...
    public static class DbInitializer
    {
        public static void Initialize(SoftSportDbContext context)
        {
            ...
            try
            {
                ...
                // Step 5: Ensure Audit Columns exist (Fix for 500 Error in Dashboard)
                var provider = context.Database.ProviderName;
                Console.WriteLine($"[INFO] Database Provider: {provider}");

                if (provider != null && provider.Contains("PostgreSQL"))
                {
                    var auditColumnsSql = @"
                    DO $$ 
                    DECLARE 
                    ...
                    BEGIN 
                        ...
                        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'recibos' AND column_name = 'usuario_modificacion') THEN
                            ALTER TABLE recibos ADD COLUMN usuario_modificacion VARCHAR(100);
                        END IF;
                ...
                Console.WriteLine("Initializing Audit Columns...");
                context.Database.ExecuteSqlRaw(auditColumnsSql);
                Console.WriteLine("Audit Columns initialized.");
                }
                else
                {
                    Console.WriteLine("[WARN] Skipping Audit Columns initialization (Not PostgreSQL)");
                }
            }
            catch (Exception ex)
            {
